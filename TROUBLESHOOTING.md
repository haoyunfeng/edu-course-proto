...
### 1. go: warning: "github.com/haoyunfeng/edu-course-proto/course/..." matched no packages
...
1. 确保 `proto/course.proto` 中的 `go_package` 选项为：
   ```protobuf
   option go_package = "github.com/haoyunfeng/edu-course-proto/course";
   ```
...
### 2. cannot find package "github.com/haoyunfeng/edu-course-proto/course"
...
2. 如果发现有类似这样的错误 replace：
   ```go
   replace github.com/haoyunfeng/edu-course-proto/course => ./course_stub
   ```
   ...
   **方案A：删除错误的 replace（推荐）**
   ...
   ```bash
   go mod edit -dropreplace github.com/haoyunfeng/edu-course-proto/course
   ```
   ...
   **方案B：使用正确的 replace**
   ...
   replace github.com/haoyunfeng/edu-course-proto => /path/to/edu-course-proto
   ```
   注意：替换的是模块路径 `github.com/haoyunfeng/edu-course-proto`，而不是包路径 `github.com/haoyunfeng/edu-course-proto/course`
...
- 包路径：`github.com/haoyunfeng/edu-course-proto/course`（用于 import）
...
# 3. 检查包路径
go list ./course/...
# 应该输出：github.com/haoyunfeng/edu-course-proto/course
